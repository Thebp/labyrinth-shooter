 ...
 */
package gruppe5.Entity;

import gruppe5.common.data.Entity;
import gruppe5.common.node.MapNode;
import static gruppe5.mapgenerator.MapGenerator.MAP_UNIT_SIZE;
...
public class MapEntityCreator {
    ...
    public static Entity createFloorEntity(int mazeX, int mazeY) {
        ...
        float x = mazeX * floorSize + (MAP_UNIT_SIZE * (NODES_IN_CORRIDOR - 1) / 2);
        float y = mazeY * floorSize + (MAP_UNIT_SIZE * (NODES_IN_CORRIDOR - 1) / 2);
    ...
    public static Entity createWallEntity(int mazeX, int mazeY, boolean[] neighbors) {
        ...
        float x = mazeX * wallSize + (MAP_UNIT_SIZE * (NODES_IN_CORRIDOR - 1) / 2);
        float y = mazeY * wallSize + (MAP_UNIT_SIZE * (NODES_IN_CORRIDOR - 1) / 2);
    ...
    public static Entity createNodeEntity(MapNode n) {
        ...
        float x = n.getX();
        float y = n.getY();
        float unit = MAP_UNIT_SIZE;

        node.setPosition(x, y);
        node.setCollidable(false);
        node.setDynamic(false);
        node.setRadius(MAP_UNIT_SIZE);
